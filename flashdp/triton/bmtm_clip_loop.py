...
from .clip_fn import get_clip_factor
...
def _mtm_prenorm_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        a = tl.load(a_ptrs, mask=offs_k[None, :] < K - k * BLOCK_SIZE_K, other=0.0).to(acc_dtype)
        b = tl.load(b_ptrs, mask=offs_k[:, None] < K - k * BLOCK_SIZE_K, other=0.0).to(acc_dtype)
...
def _mat_clip_kernel(
    ...
    clip_factor = get_clip_factor(norm, clip_args_ptr)
